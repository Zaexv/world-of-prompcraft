...
function varyColor(hex: number, amount: number): number {
  const r = (hex >> 16) & 255;
  const g = (hex >> 8) & 255;
  const b = hex & 255;
  const adjust = (val: number) => Math.max(0, Math.min(255, val + 255 * amount));
...
export function buildProceduralMesh(
  ...
  // Apply slight variety to colors and dimensions
  a.bodyColor = varyColor(a.bodyColor, rng(1) * 0.1 - 0.05);
  a.headColor = varyColor(a.headColor, rng(2) * 0.1 - 0.05);
  a.bodyWidth *= 0.95 + rng(3) * 0.1;
  a.bodyDepth *= 0.95 + rng(4) * 0.1;
  ...
  // ── Arms (pivot group at the shoulder so swings/waves rotate from the
  //    joint rather than the mid-arm — mirrors the leg hip-pivot below) ──
  ...
  const armHalfHeight = 0.33;
  const shoulderY = NPC_Y_ARM + armHalfHeight; // keeps the rest pose identical

  const lArmPivot = new THREE.Group();
  lArmPivot.name = 'leftArm';
  lArmPivot.position.set(-ARM_X, shoulderY, 0);
  group.add(lArmPivot);
  const lArm = new THREE.Mesh(armGeo, armMat);
  lArm.position.y = -armHalfHeight;
  ...
  rArmPivot.position.set(ARM_X, shoulderY, 0);
  group.add(rArmPivot);
  const rArm = new THREE.Mesh(armGeo, armMat);
  rArm.position.y = -armHalfHeight;

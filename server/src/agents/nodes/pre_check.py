...
_FOLLOW_PATTERNS = re.compile(r"\b(follow|come with me|accompany)\b", re.IGNORECASE)
_STOP_PATTERNS = re.compile(r"\b(stop|wait|stay here)\b", re.IGNORECASE)
...
def make_pre_check_node(llm: BaseChatModel) -> Any:
    ...
    async def pre_check_node(state: NPCAgentState) -> dict[str, Any]:
        ...
        if _is_direct_command(prompt_lower):
            ...
            if _FOLLOW_PATTERNS.search(prompt_lower):
                logger.info("Pre-check: Fast-routed to FOLLOW.")
                return {
                    "fast_intent": "social",
                    "messages": [
                        AIMessage(
                            content="Very well, I shall follow your lead.",
                            tool_calls=[
                                {
                                    "name": "follow_player",
            ...
            if _STOP_PATTERNS.search(prompt_lower):
                logger.info("Pre-check: Fast-routed to STOP.")
                return {
                    "fast_intent": "social",
                    "messages": [
                        AIMessage(
                            content="I will wait here then.",
                            tool_calls=[
                                {
                                    "name": "stop_following",
                                    "args": {},

...
- [ ] **No Texture Stretching:** Large or non-uniform primitives MUST use `applyWorldTiling(group, material)` to drive UVs from world units. **Custom BufferGeometry MUST explicitly define a `uv` attribute based on world units (e.g., 1 unit = 1 meter) rather than a 0-1 range, or textures will stretch.**
  - **Flat cylinder/cone CAPS:** `applyWorldTiling` tiles a cylinder/cone by circumference (U) × height (V) — correct for tall side walls but it smears that scaling onto the flat top/bottom caps. Hidden on a tall tower, but on a **low disc/drum** (amphitheatre floor, plinth top, fountain basin) the cap is the visible face and reads as stretched streaks. After tiling, re-map cap UVs (vertices with `|normal.y| > 0.9`) to world XZ at the same density.
...
### 2. Performance & Optimization (the renderer is DRAW-CALL bound)
...
- [ ] **Minimize materials = minimize draws:** Geometry auto-merges to **one draw call per material** (`mergeStatic.ts`, run by `buildMesh`/`withLOD`), so sub-mesh count is not the concern — **distinct material count is.** Flag any `new THREE.Mesh*Material(...)` created per item inside a loop (a unique colour per book/tile/pot/crate); these must pull from a small shared/cached palette. A building should have ~3–8 distinct materials, not dozens. (A dense town hit 5000+ draws and froze a high-end GPU purely on draw count, with only ~170k triangles.)
- [ ] **Merge-friendly geometry:** Only opaque, single-material, visible meshes merge. `transparent` meshes, multi-material meshes (material arrays), `InstancedMesh`/`SkinnedMesh`, and invisible collider proxies each stay a separate draw. Flag opaque geometry needlessly marked `transparent`, or material arrays used where one material would do.
- [ ] **NO per-mesh lights (CRITICAL):** A `THREE.PointLight`/`SpotLight`/`HemisphereLight` created in a mesh's `build()` changes the global light count → Three.js **recompiles every material in the game** (100–600 ms stalls) and adds per-fragment cost forever. Flag ANY light constructor in a mesh file. Glow must be **emissive material + bloom**; genuine local illumination must go through `addLightEmitter(...)` from `scene/PointLightPool` — never `new THREE.PointLight`.
- [ ] **Instancing for repeated identical meshes:** A mesh scattered many times whose geometry is identical per placement (trees, rocks, posts — only pos/rot/scale vary) should declare `static readonly instanceable = true` so the populator batches all copies in a chunk into one `InstancedMesh` per material. Flag a common scattered prop/tree missing it — but ONLY if `build()` does not vary shape by `position`/`rng` (per-placement variation rules out instancing).
